...
// one object save clients
    ...
    io.sockets.on('connection', function (socket) {
        socket.on('ask', function (msg) {
            ...
            if (msg.to && admins.hasOwnProperty(msg.to)) {
                //check if msg to is in the admins

                io.sockets.socket(msg.to).emit('ask', { from: socket.id, body: msg.body });
            } else {
                //find already connected admin
                var connected_admin = _.find(admins, function (single) {
                    return single.clients_connected.indexOf(socket.id) > -1;
                });
                if (connected_admin) {
                    io.sockets.socket(connected_admin.id).emit('ask', { from: socket.id, body: msg.body });
                    return;
                }
        ...
        socket.on("unregister", function (msg) {
            if (msg.role === 1) {
                ...
                if (clients.hasOwnProperty(socket.id)) {
                    ...
                    var index;
                    var adminContainsClient = _.find(admins, function (single) {
                        index = single.clients_connected.indexOf(socket.id);
                        return index > -1;
                    });
                    if (adminContainsClient) {
                        adminContainsClient.clients_connected.slice(index, 1);
                    }
                }
            }
        });
